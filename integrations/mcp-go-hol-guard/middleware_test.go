...
func TestDenyAndFailureNeverInvokeHandler(t *testing.T) {
	for _, tc := range []struct {
		...
		{"provider panic", DecisionProviderFunc(func(context.Context, ToolCall) (Decision, error) { panic("provider bug") })},
		{"ambiguous", DecisionProviderFunc(func(context.Context, ToolCall) (Decision, error) { return Decision{}, nil })},
	} {
		t.Run(tc.name, func(t *testing.T) {
			var calls atomic.Int32
			h := Middleware(tc.provider)(func(context.Context, mcp.CallToolRequest) (*mcp.CallToolResult, error) {
				calls.Add(1)
			...
			if !result.IsError {
				t.Fatal("expected fail-closed tool result")
			}
			if calls.Load() != 0 {
				t.Fatalf("downstream executed %d times", calls.Load())
			}
		})
	}
}
...
func TestAllowInvokesHandlerExactlyOnce(t *testing.T) {
	...
	h := Middleware(provider)(func(context.Context, mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		calls.Add(1)
		return mcp.NewToolResultText("executed"), nil
	})
	result, err := h(t.Context(), request())
...
func TestRealServerDenyMeansZeroToolExecution(t *testing.T) {
	...
	srv.AddTool(mcp.NewTool("destructive", mcp.WithString("command")), func(context.Context, mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		calls.Add(1)
		return mcp.NewToolResultText("executed"), nil
	})
	...
	if !result.IsError {
		t.Fatal("expected blocked MCP result")
	}
	if calls.Load() != 0 {
		t.Fatalf("real server tool executed %d times", calls.Load())
	}
}
