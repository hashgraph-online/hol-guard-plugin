...
3. The Guard launcher builds a minimal allowlisted environment. It prepends only the already-running DSH Node runtime directory as an explicit interpreter trust anchor, then adds absolute, non-temporary, non-workspace, owner-safe PATH entries.
4. The Guard executable is resolved to an absolute path outside the active workspace. Every `hol-guard` candidate, including any candidate in the Node runtime directory, must independently pass the regular-file, executable, ownership, and mode checks.
5. The exact payload is reviewed by `hol-guard guard hook --harness dsh` under a process-tree deadline.
6. Hard-deny signals dominate review and allow across same-layer fields, nested wrappers, and sibling response branches.
7. An authoritative `ask` or review decision is routed through DSH's native one-time approval service.
8. Rejection, cancellation, a missing approval service, a headless call without an agent, an unknown approval outcome, or an approval transport failure is latched as deny.
9. Before downstream pre-execute policy runs, HOL Guard makes the execution's own readonly identity fields non-writable and non-configurable.
10. After downstream policy returns, HOL Guard reserializes the call. Any change to the reviewed name, arguments, IDs, or workspace becomes a denial.
11. The monotonic `ctx.tools.guard()` callback revalidates the serialized call and permits only a completed, unchanged allow. A missing latch, mutation, or denial returns a final reason and prevents dispatch.
...
| Running DSH Node runtime missing, non-executable, or resolved inside the workspace or system temporary root | Deny while constructing the child environment. |
...
- A bare `hol-guard` command is resolved only through absolute PATH entries outside the workspace and system temporary root.
- The directory containing the realpath of the already-running `process.execPath` is prepended as an explicit trust anchor so packaged `#!/usr/bin/env node` launchers execute under the same Node runtime that is already hosting DSH.
- The Node trust anchor does not bypass Guard executable validation. A `hol-guard` candidate found there is still accepted only if its own realpath and full path chain pass the ordinary file, workspace, temporary-root, ownership, mode, and executable checks.
- On Unix, the Guard executable and every directory in its resolved path chain must be owned by the current user or root and must not be writable by another user or group. Root-owned sticky temporary directories are not themselves treated as executable sources.
- Other PATH entries inside the active workspace, relative entries, empty entries, temporary entries, missing directories, and owner-unsafe directories are removed.
...
- the already-running DSH Node runtime directory followed by a sanitized owner-safe PATH;
...
These controls prevent a repository from placing a fake `hol-guard` executable in the workspace, selecting a temporary fake through PATH, replacing the Python/native process through inherited environment variables, or leaking unrelated process credentials into the local policy decision. Preserving the active Node runtime directory does not restore general PATH inheritance.
...
The process trust checks establish safe default lookup, ownership/mode validation on Unix, inherited-environment isolation, and continuity with the already-running DSH Node runtime. They do not claim protection if the user's trusted installed executable, explicit profile configuration, operating-system account, current DSH Node runtime, or owner-safe installation directories have already been compromised. Windows uses the workspace, temporary-root, realpath, file-type, and absolute-path checks but does not claim Unix ownership/mode semantics.
...
- process-trust tests proving the minimal environment allowlist, secret/injection omission, active Node runtime anchoring, workspace and temporary PATH exclusion, owner-safe absolute Guard resolution, explicit-path validation, Guard-home validation, and runner isolation;
- manifest validation requiring the DSH `tools` service, monotonic guard registration, execution-binding logic, the packaged process-trust module, and the active-runtime trust anchor;
